...
That fetches `/__keel/schema` and writes blank Svelte pages (React is a
follow-up). Gradle `generateKeelTypes` remains the offline classpath scan
when no host is running. The live schema carries real page paths; the
classpath scan uses placeholder `/__keel-typegen/…` paths because
`@KeelType` does not store the host route.
...
Full JSON Schema for payload shapes is a later extension — the emitter
already walks `SerialDescriptor`.
